...
package com.mycim.logging.processor.validation;

import com.mycim.logging.processor.model.*;
import com.mycim.logging.processor.util.ElementHelper;
...
import static com.mycim.logging.processor.validation.ValidationMessageFactory.createError;
import static com.mycim.logging.processor.validation.ValidationMessageFactory.createWarning;
...
public final class Validator {
    ...
    private static final Collection<String> AVAILABLE_LANGUAGES = new HashSet<>(
    ...
    public final Collection<ValidationMessage> validate(final MessageInterface messageInterface) {
        ...
        if (messageInterface.isAnnotatedWith(MessageBundle.class)) {
            ...
            messages.add(
                    createError(messageInterface, "Message interface %s is not a message bundle or message logger.",
                                messageInterface.name()));
        }

        // Check the locale is in the list of available locales
        if (locale != null && !locale.isEmpty() &&
                !AVAILABLE_LANGUAGES.contains(Locale.forLanguageTag(locale).getLanguage())) {
        ...
        for (MessageMethod messageMethod : messageMethods) {
            ...
            for (ThrowableType throwableType : messageMethod.thrownTypes()) {
                if (throwableType.isChecked()) {
                    messages.add(
                            createError(messageMethod, "Interface message methods cannot throw checked exceptions."));
                }
            }
            final MessageMethod.Message message = messageMethod.message();
            if (message == null) {
                messages.add(createError(messageMethod,
                                         "All message bundles and message logger message methods must have or inherit" +
                                                 " a message."));
                continue;
            }
            ...
            if (message.hasId()) {
                ...
                if (message.id() < 0) {
                    messages.add(createError(messageMethod,
                                             "Message id %d is invalid. Must be greater than 0 or inherit another " +
                                                     "valid id.",
                                             message.id()));
                } else {
            ...
            if (formatValidator.isValid()) {
                ...
                if (messageMethod.formatParameterCount() != formatValidator.argumentCount()) {
                    messages.add(createError(messageMethod,
                                             "Parameter count does not match for format '%s'. Required: %d Provided: " +
                                                     "%d",
                ...
                for (Parameter parameter : messageMethod.parameters()) {
                    ...
                    if (parameter.isAnnotatedWith(Pos.class)) {
                        ...
                        if (transforms != null && transforms.length > 0) {
                            if (pos.value().length != transforms.length) {
                                messages.add(createError(parameter,
                                                         "Positional parameters with transforms must have an equal " +
                                                                 "number of positions and transforms."));
                            } else {
                        ...
                        for (int position : pos.value()) {
                            if (usedPositions.contains(position)) {
                                messages.add(createError(parameter, "Position '%d' already used for this parameter.",
                                                         position));
                            } else {
                                usedPositions.add(position);
                            }
                            if (positions.containsKey(position)) {
                                messages.add(createError(parameter, "Position '%d' already defined on parameter '%s'",
                                                         position, positions.get(position).name()));
                            } else {
                    ...
                    // Validate the @Suppressed parameter is on a message bundle, the return type is an exception and
                    // the parameter is an exception
                    if (parameter.isAnnotatedWith(Suppressed.class)) {
                        if (!messageMethod.returnType().isThrowable()) {
                            messages.add(createError(messageMethod,
                                                     "The @Suppressed parameter annotation can only be used with " +
                                                             "message bundle methods that return an exception."));
                        }
                        if (!isTypeAssignableFrom(parameter, Throwable.class)) {
                            messages.add(createError(parameter,
                                                     "The parameter annotated with @Suppressed must be assignable to " +
                                                             "a Throwable type."));
                        }
                    }
                }
                ...
                if (validatePositions) {
                    for (int i = 0; i < messageMethod.formatParameterCount(); i++) {
                        ...
                        if (!positions.containsKey(positionIndex)) {
                            messages.add(createError(messageMethod, "Missing parameter with position '%d' defined.",
                                                     positionIndex));
                        }
                    }
                }
            ...
            if (!messageMethod.inheritsMessage()) {
                ...
                if (methodNames.containsKey(key)) {
                    ...
                    messages.add(
                            createError(messageMethod, "Only one message with the same format parameters is allowed."));
                } else {
    ...
    private void validateTransform(final List<ValidationMessage> messages, final Parameter parameter,
        ...
        if (parameter.isPrimitive()) {
            ...
            messages.add(
                    createError(parameter, "Transform type '%s' not allowed with type '%s'", TransformType.GET_CLASS,
                                TransformType.SIZE));
        } else if (transformTypes.contains(TransformType.HASH_CODE) && transformTypes.contains(TransformType.SIZE)) {
            messages.add(
                    createError(parameter, "Transform type '%s' not allowed with type '%s'", TransformType.HASH_CODE,
                                TransformType.SIZE));
        } else if (transformTypes.contains(TransformType.IDENTITY_HASH_CODE) &&
                transformTypes.contains(TransformType.SIZE)) {
            ...
            if (!(parameter.isArray() || parameter.isVarArgs() || parameter.isSubtypeOf(Map.class) ||
                ...
                messages.add(createError(parameter, "Invalid type (%s) for %s. Must be an array, %s, %s or %s.",
                                         parameter.asType(), TransformType.SIZE, Collection.class.getName(),
                                         Map.class.getName(), CharSequence.class.getName()));
            }
        }
    }
        ...
        for (Parameter parameter : messageMethod.parameters()) {
            ...
            if (parameter.isAnnotatedWith(LoggingClass.class)) {
                if (!parameter.isSameAs(Class.class)) {
                    messages.add(createError(parameter,
                                             "Parameter %s annotated with @LoggingClass on method %s must be of type " +
                                                     "%s.",
                ...
                if (isFunction) {
                    if (size == 2) {
                        ...
                        if (!isTypeAssignableFrom(message, String.class)) {
                            messages.add(
                                    createError(parameter, "The type %s must be assignable to a String.", message));
                        }
                        ...
                        messages.add(createError(parameter, "The type parameters could not be validated for the " +
                                "function. The first type argument of the function must be a String and the second " +
                                "type " +
                    ...
                    if (size == 3) {
                        ...
                        if (!isTypeAssignableFrom(first, String.class) &&
                            ...
                            messages.add(createError(parameter, "The first type type parameter for %s " +
                                                             "must be assignable to a String or a super type of a " +
                                                             "Throwable.",
                                                     parameter.asType()));
                        }
                        ...
                        if (messageMethod.hasCause()) {
                            ...
                            if (types.isSubtype(first, ElementHelper.toType(elements, Throwable.class)) &&
                                ...
                                messages.add(createError(parameter,
                                                         "The first parameter type, %s, of the BiFunction must be " +
                                                                 "assignable to the cause %s.",
                            ...
                            if (types.isSubtype(second, ElementHelper.toType(elements, Throwable.class)) &&
                                ...
                                messages.add(createError(parameter,
                                                         "The second parameter type, %s, of the BiFunction must be " +
                                                                 "assignable to the cause %s.",
                        ...
                        messages.add(createError(parameter, "The type parameters could not be validated for the " +
                                ...
                                "a super type of %s.", requiredReturnType));
                    }
                } else {
                    messages.add(createError(parameter, "Parameter annotated with %s must be a %s or %s type",
                                             Producer.class.getName(), Function.class.getName(),
                                             BiFunction.class.getName()));
                }

                if (producerFound) {
                    messages.add(createError(messageMethod, "Only one parameter is allowed to be annotated with %s.",
    ...
    private Collection<ValidationMessage> validateBundleMethod(final MessageMethod messageMethod) {
        ...
        if (returnTypeMirror.getKind() == TypeKind.VOID || returnTypeMirror.getKind().isPrimitive()) {
            messages.add(createError(messageMethod,
                                     "Message bundle messageMethod %s has an invalid return type. Cannot be void or a" +
                                             " primitive.",
                                     messageMethod.name()));
            ...
            if (throwableReturnType.useConstructionParameters()) {
                ...
                if (signature != null) {
                    final List<TypeMirror> args = ElementHelper.getClassArrayAnnotationValue(messageMethod,
                    ...
                    if (messageIndex < 0) {
                        messages.add(createError(messageMethod,
                                                 "A messageIndex of 0 or greater is required. Value %d is invalid.",
                                                 messageIndex));
                    }
                }
                ...
                messages.add(createError(messageMethod,
                                         "MessageMethod does not have an usable constructor for the return type %s.",
                                         returnType.name()));
            } else {
                ...
                if (!messageMethod.parametersAnnotatedWith(Producer.class).isEmpty()) {
                    ...
                    if (!isTypeAssignableFrom(erasure, Throwable.class)) {
                        messages.add(createError(messageMethod,
                                                 "The return type must be a subclass of a java.lang.Throwable"));
                    }
                    ...
                    messages.add(createWarning(messageMethod,
                                               "The message cannot be set via the throwable constructor and will be " +
                                                       "ignored."));
                }
            }
        } else {
            if (!isTypeAssignableFrom(resolvedReturnType, String.class) && !returnType.isThrowable()) {
                messages.add(createError(messageMethod,
                                         "Message bundle method (%s) has an invalid return type of %s. " +
                                                 "Return types must be a String, a subtype of Throwable or a java" +
                                                 ".util.function.Supplier which " +
                                                 "returns a String or a subtype of Throwable.", messageMethod.name(),
                                         returnTypeMirror));
            }
            if (messageMethod.isAnnotatedWith(ConstructType.class)) {
                messages.add(createError(messageMethod,
                                         "ConstructType annotation requires a throwable or supplier which produces a " +
