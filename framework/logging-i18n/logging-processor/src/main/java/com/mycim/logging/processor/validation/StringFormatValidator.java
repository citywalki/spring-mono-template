...
public final class StringFormatValidator extends AbstractFormatValidator {
    ...
    public static StringFormatValidator withTranslation(final String format, final String translationFormat) {
        ...
        try {
            ...
            if (translationResult.isValid()) {
                ...
                translationResult.setDetailMessage("Format '%s' appears to be invalid. Error: %s", format,
                                                   e.getMessage());
            }
        }
    ...
    private void validate() {
        if (!format.equalsIgnoreCase(asFormat())) {
            ...
            setDetailMessage(
                    "The original is '%s' and the reconstructed format is '%s'. This is likely an internal error and " +
                            "should be reported.",
                    format, asFormat());
        } else {
            ...
            if (valid) {
                try {
                    ...
                    valid = false;
                    setSummaryMessage("Invalid format for '%s' with parameters '%s'. java.util.Formatter Error: %s",
                                      format, paramMap, e.getMessage());
