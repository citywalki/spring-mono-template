...
import com.mycim.logging.processor.apt.ProcessingException;
import com.mycim.logging.processor.model.MessageInterface;
import com.mycim.logging.processor.model.MessageMethod;
import com.mycim.logging.processor.model.Parameter;
import com.mycim.logging.processor.model.ThrowableType;
...
abstract class ImplementationClassModel extends ClassModel {
        ...
        switch (message.format()) {
            ...
            case PRINTF: {
                ...
                formatterCall = formatter
                        .call("format")
                        .arg(localeGetter)
                        .arg(JExprs.call(messageMethod.messageMethodName()));
                break;
            }
            default:
                formatterCall = JExprs.call(messageMethod.messageMethodName());
    ...
    JAssignableExpr createTransformVar(final List<String> parameterNames, final JBlock methodBody,
                                       final Parameter param, final JExpr var) {
        return createTransformVar(parameterNames, methodBody, param, param.getAnnotation(Transform.class), var);
    }

    JAssignableExpr createTransformVar(final List<String> parameterNames, final JBlock methodBody,
                                       final Parameter param, final Transform transform, final JExpr var) {
        ...
        if (transformTypes.contains(TransformType.GET_CLASS)) {
            ...
            if (transformTypes.size() == 1) {
                ...
                if (transformTypes.contains(TransformType.HASH_CODE)) {
                    ...
                    throw new IllegalStateException(
                            String.format("Invalid transform type combination: %s", transformTypes));
                }
            }
            ...
            if (param.isArray() || param.isVarArgs()) {
                ...
                throw new IllegalStateException(
                        String.format("Invalid type for %s. Must be an array, %s, %s or %s.", TransformType.SIZE,
                                      Collection.class.getName(), Map.class.getName(), CharSequence.class.getName()));
            }
    ...
    private JExpr createReturnType(final MessageMethod messageMethod, final JBlock body, final JCall format,
                                   final Map<String, JParamDeclaration> fields,
                                   final Map<String, JParamDeclaration> properties) {
        ...
        addDefultProperties(messageMethod,
                            ElementHelper.getAnnotations(messageMethod, Properties.class, Property.class), body,
                            $v(resultField), false);
        ...
        final JType arrays = $t(Arrays.class);
        sourceFile._import(arrays);
        final JVarDeclaration st = body.var(FINAL, $t(StackTraceElement.class).array(), "st",
                                            $v(resultField).call("getStackTrace"));
        body.add($v(resultField)
                         .call("setStackTrace")
        ...
        for (Parameter p : suppressed) {
            if (p.isArray() || p.isVarArgs()) {
                ...
                // TODO (jrp) the " " can be removed after an upgrade to JDeparser2; this is a workaround for a
                //  formatting bug
                body
                        .forEach(0, JTypes.typeOf(((ArrayType) p.asType()).getComponentType()), " " + name,
                                 $v(p.name()))
                ...
                body.add($v(p.name())
                                 ...
                                 .arg(JExprs
                                              .lambda()
                                              .param(name)
                                              .body($v(resultField).call("addSuppressed").arg($v(name)))));
            } else {
    ...
    private void addDefultProperties(final MessageMethod messageMethod, final Collection<AnnotationMirror> annotations,
        ...
        for (AnnotationMirror propertyAnnotation : annotations) {
            ...
            for (Map.Entry<? extends ExecutableElement, ? extends AnnotationValue> entry : propertyAnnotation
                    .getElementValues()
                    .entrySet()) {
            ...
            if (field) {
                ...
                body.add(resultField
                                 .call("set" + Character.toUpperCase(name.charAt(0)) + name.substring(1))
                                 .arg(resultValue));
            }
        }
    }
    ...
    private JCall getFormatMethod(final JClassDef classDef, final JCall localeGetter) {
        ...
        if (messageFormatMethodGenerated.compareAndSet(false, true)) {
            ...
            final JCall initializer = formatterType._new().arg($v(format)).arg(localeGetter);
            ...
            body._return($v(formatter)
                                 .call("format")
                                 .arg($v(args))
                                 .arg($t(StringBuffer.class)._new())
                                 .arg($t(FieldPosition.class)._new().arg(JExpr.ZERO))
                                 .call("toString"));
        }
