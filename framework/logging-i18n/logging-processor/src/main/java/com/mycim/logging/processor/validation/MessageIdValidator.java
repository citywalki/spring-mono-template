...
public final class MessageIdValidator {
                ...
                if (interfaceMessageMethods.contains(messageMethod)) {
                    ...
                    if (invalidId) {
                        ...
                        messages.add(createError(messageMethod,
                                                 "Message id %d on method %s is not within the valid range: %s", id,
                                                 messageMethod.name(), ranges.toString()));
                    }
                }
                    ...
                    if (usedMessageIds.containsKey(key)) {
                        ...
                        if (!previousMethod.name().equals(messageMethod.name())) {
                            messages.add(createError(previousMethod,
                                                     "Message id %s is not unique for messageMethod %s with project " +
                                                             "code %s.",
                                                     id, previousMethod.name(), projectCode));
                            messages.add(createError(messageMethod,
                                                     "Message id %s is not unique for messageMethod %s with project " +
                                                             "code %s.",
                                                     id, messageMethod.name(), projectCode));
                        }
                    } else {
    ...
    private static class MessageKey implements Comparable<MessageKey> {
        ...
        @Override
        public int hashCode() {
            return HashCodeBuilder.builder().add(projectCode).add(id).toHashCode();
        }
        ...
        public int compareTo(final MessageKey other) {
            return Comparison.begin().compare(projectCode, other.projectCode).compare(id, other.id).result();
        }
