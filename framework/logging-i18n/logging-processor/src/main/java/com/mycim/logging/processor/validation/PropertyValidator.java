...
class PropertyValidator {
    ...
    static Collection<ValidationMessage> validate(final ProcessingEnvironment processingEnv,
        ...
        if (continueValidation) {
            ...
            final TypeMirror returnType = processingEnv
                    .getTypeUtils()
                    .erasure(messageMethod.returnType().resolvedType());
            final List<ValidationMessage> result = new ArrayList<>();
            if (returnType.getKind() == TypeKind.DECLARED) {
                final PropertyValidator validator = new PropertyValidator(processingEnv, messageMethod, returnType,
                                                                          result);
                validator.validate();
            } else {
    ...
    private void validate() {
        ...
        ElementHelper
                .getAnnotations(method, Properties.class, Property.class)
                .forEach(a -> validateAnnotation(a, methods));
        ...
        for (Parameter parameter : method.parametersAnnotatedWith(Field.class)) {
            ...
            if (!assignablePropertyFound(valueType, propertyTypes)) {
                messages.add(
                        createError(parameter, "No target field found in %s with name %s with type %s.", resultType,
    ...
    private void validateCommonAnnotation(final Parameter parameter, final Class<? extends Annotation> annotation) {
        ...
        if (annotations.size() != 1) {
            messages.add(createError(parameter, "Parameters can contain only a single @%s annotation.",
                                     annotation.getName()));
        } else {
                ...
                for (Map.Entry<? extends ExecutableElement, ? extends AnnotationValue> entry : map.entrySet()) {
                    ...
                    if (!"name".contentEquals(attribute.getSimpleName())) {
                        messages.add(createError(parameter, annotationMirror, attributeValue,
                                                 "Default values are not allowed for parameters annotated with @%s. %s",
                                                 annotation.getName(), annotationMirror));
                    }
                }
            }
        }
    ...
    private void validateAnnotation(final AnnotationMirror annotationMirror,
                                    final Map<String, Set<TypeMirror>> properties) {
        // We should have a name value and one other value
        final Map<? extends ExecutableElement, ? extends AnnotationValue> map = annotationMirror.getElementValues();
        final int size = map.size();
        if (size < 2) {
            messages.add(createError(method, annotationMirror,
                                     "The name attribute and at least one default value are required: %s",
                                     annotationMirror));
        } else if (size > 2) {
            messages.add(createError(method, annotationMirror,
                                     "Only the name attribute and one default attribute are allowed to be defined: %s",
                                     annotationMirror));
        } else {
            // Look for the name attribute and a single value
            String name = null;
            ...
            if (name == null) {
                messages.add(createError(method, annotationMirror, "The name attribute is required on %s",
                                         annotationMirror));
            } else if (value == null) {
                messages.add(
                        createError(method, annotationMirror, "No value could be determined for %s", annotationMirror));
            } else {
                ...
                if (propertyTypes == null) {
                    ...
                    final TypeMirror defaultValueType = value.accept(ValueTypeAnnotationValueVisitor.INSTANCE,
                                                                     elements);
                    if (!assignablePropertyFound(defaultValueType, propertyTypes)) {
                        messages.add(createError(method, annotationMirror, value,
                                                 "Expected property with type %s found with type %s", defaultValueType,
                                                 propertyTypes));
                    }
                }
            }
