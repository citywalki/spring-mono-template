...
final class ReturnTypeFactory {
    ...
    private static class AptReturnType extends AbstractClassType implements ReturnType {
        ...
        AptReturnType(final ProcessingEnvironment processingEnv, final TypeMirror returnType,
                      final MessageMethod method) {
            ...
            throwableType = null;
            if (types.isSubtype(types.erasure(returnType),
                                types.erasure(ElementHelper.toType(elements, Supplier.class)))) {
        ...
        private void init() {
            if (isThrowable()) {
                ...
                if (method.isAnnotatedWith(ConstructType.class)) {
                    final TypeElement constructTypeValue = ElementHelper.getClassAnnotationValue(method,
                                                                                                 ConstructType.class);
                    // Shouldn't be null
