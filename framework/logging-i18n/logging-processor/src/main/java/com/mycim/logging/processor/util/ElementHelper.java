...
package com.mycim.logging.processor.util;
...
public final class ElementHelper {
     ...
     */
    public static TypeElement getClassAnnotationValue(final Element element,
    ...
    public static List<TypeMirror> getClassArrayAnnotationValue(final Element element,
                                                                final Class<? extends Annotation> annotation,
                                                                @SuppressWarnings(
                                                                        "SameParameterValue") final String attributeName) {
        for (AnnotationMirror mirror : element.getAnnotationMirrors()) {
            final DeclaredType annotationType = mirror.getAnnotationType();
            if (annotationType.toString().equals(annotation.getName())) {
                final Map<? extends ExecutableElement, ? extends AnnotationValue> map = mirror.getElementValues();
                for (ExecutableElement key : map.keySet()) {
                    if (key.getSimpleName().contentEquals(attributeName)) {
                        @SuppressWarnings(
                                "unchecked") final List<AnnotationValue> annotationValues = (List<AnnotationValue>) map
                                .get(key)
                                .getValue();
     ...
     * @param groupedAnnotation the grouped annotation, e.g. collector for repeatable annotations, or {@code null} if () {
    ...
    public static Collection<AnnotationMirror> getAnnotations(final Element element,
                                                              final Class<? extends Annotation> groupedAnnotation,
                                                              final Class<? extends Annotation> annotation) {
    ...
    private static Collection<AnnotationMirror> getContainingAnnotations(final AnnotationMirror annotation) {
        ...
        final Map<? extends ExecutableElement, ? extends AnnotationValue> childAnnotations =
