...
public class SquigglyParser {
    ...
    public List<SquigglyNode> parse(String filter) {
        ...
        SquigglyExpressionLexer lexer = ThrowingErrorListener.overwrite(
                new SquigglyExpressionLexer(new ANTLRInputStream(filter)));
        SquigglyExpressionParser parser = ThrowingErrorListener.overwrite(
                new SquigglyExpressionParser(new CommonTokenStream(lexer)));
    ...
    private class Visitor extends SquigglyExpressionBaseVisitor<List<SquigglyNode>> {
        ...
        private void handleExpression(SquigglyExpressionParser.ExpressionContext ctx, MutableNode parent) {
            ...
            if (ctx.field() != null) {
                ...
                names = Collections.singletonList(
                        createName(ctx.dot_path().field().get(ctx.dot_path().field().size() - 1)));
        ...
        private void handleNegatedExpression(SquigglyExpressionParser.Negated_expressionContext ctx,
                                             MutableNode parent) {
            if (ctx.field() != null) {
