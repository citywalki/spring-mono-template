...
public class SquigglyPropertyFilter extends SimpleBeanPropertyFilter {
    ...
    private static final List<SquigglyNode> BASE_VIEW_NODES = Collections.singletonList(
            new SquigglyNode(new ExactName(PropertyView.BASE_VIEW), Collections.<SquigglyNode>emptyList(), false, true,
                             false));
        ...
        for (int i = 0; i < pathSize; i++) {
            ...
            if (viewNode != null && !viewNode.isSquiggly()) {
                ...
                if (i < lastIdx && nodes.isEmpty() && !match.isEmptyNested() &&
                        SquigglyConfig.isFilterImplicitlyIncludeBaseFields()) {
